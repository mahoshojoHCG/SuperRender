...
/// <summary>
/// Manages a Vulkan compute pipeline for combined dequantization + IDCT.
/// Uses 3 SSBOs: raw DCT coefficients (input), quantization table (input), pixel values (output).
/// Each workgroup (64 threads) processes one 8x8 DCT block.
/// </summary>
public sealed unsafe class DequantIdctPipeline : IDisposable
{
    ...
    public DequantIdctPipeline(VulkanContext ctx, ILogger? logger = null)
    {
        _ctx = ctx;
        _logger = logger;
    ...
    {
        var bindings = new DescriptorSetLayoutBinding[3];

        // Binding 0: raw DCT coefficients SSBO (readonly)
        ...
        // Binding 1: quantization table SSBO (readonly)
        ...
        // Binding 2: output pixel values SSBO (writeonly)
        ...
        fixed (DescriptorSetLayoutBinding* pBindings = bindings)
        {
            ...
            if (result != Result.Success)
                throw new InvalidOperationException($"Failed to create dequant+IDCT descriptor set layout: {result}");
    ...
    private void CreatePipelineLayout()
    {
        ...
        if (result != Result.Success)
            throw new InvalidOperationException($"Failed to create dequant+IDCT pipeline layout: {result}");
    ...
    private void CreateComputePipeline()
    {
        var shaderCode = ShaderCompiler.LoadOrCompileComputeShader(
            "Resources.Shaders.idct_dequant.comp.spv",
            "Shaders.idct_dequant.comp.glsl", _logger);

        if (shaderCode is null)
        {
            _logger?.LogWarning("Could not load dequant+IDCT compute shader. Combined GPU path unavailable.");
            return;
        }
        ...
        fixed (byte* pCode = shaderCode)
        {
            ...
            var result = _ctx.Vk.CreateShaderModule(_ctx.Device, in moduleInfo, null,
                out shaderModule);
            if (result != Result.Success)
                throw new InvalidOperationException($"Failed to create dequant+IDCT shader module: {result}");
        }
        ...
        fixed (byte* pEntry = entryPoint)
        {
            ...
            if (result != Result.Success)
                throw new InvalidOperationException($"Failed to create dequant+IDCT compute pipeline: {result}");
            Pipeline = pipeline;
        }
