...
public sealed class JsObjectGenerator : IIncrementalGenerator
{
    ...
    private const string JsGlobalInstallAttr = "JsGlobalInstallAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var globalInstalls = context.SyntaxProvider
            .ForAttributeWithMetadataName(
                $"{Ns}.{JsGlobalInstallAttr}",
                static (node, _) => node is ClassDeclarationSyntax cds
                    && cds.Modifiers.Any(m => m.ValueText == "partial"),
                static (ctx, _) => CollectGlobalInstall(ctx))
            .Where(static x => x is not null)
            .Select(static (x, _) => x!);

        context.RegisterSourceOutput(globalInstalls, static (ctx, model) => EmitStandaloneGlobalInstall(ctx, model));
    }
    ...
    private static JsObjectModel? Collect(GeneratorAttributeSyntaxContext ctx)
    {
        ...
        string? globalInstallName = null;
        foreach (var clsAttr in cls.GetAttributes())
        {
            if (clsAttr.AttributeClass?.Name == JsGlobalInstallAttr
                && clsAttr.ConstructorArguments.Length > 0
                && clsAttr.ConstructorArguments[0].Value is string gin)
            {
                globalInstallName = gin;
            }
        }
    ...
    private static void Emit(SourceProductionContext ctx, JsObjectModel model, string? targetDir)
    {
        ...
        if (model.Methods.Length == 0 && model.Properties.Length == 0
            ...
            && model.GlobalInstallName is null)
        {
            return;
        }
        ...
        if (model.GlobalInstallName is not null && model.Constructor is null)
        {
            EmitGlobalInstall(ctx, model);
        }
    ...
    private static void EmitGlobalInstall(SourceProductionContext ctx, JsObjectModel model)
    {
        var name = model.GlobalInstallName!;
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("#nullable enable");
        sb.AppendLine("using SuperRender.EcmaScript.Runtime;");
        ...
        sb.Append("partial class ").Append(model.ClassName).AppendLine(" : IJsInstallable");
        sb.AppendLine("{");
        sb.AppendLine("    static void IJsInstallable.Install(Realm realm)");
        sb.AppendLine("    {");
        sb.Append("        var instance = new ").Append(model.ClassName).AppendLine("(realm);");
        sb.AppendLine("        __OnGlobalInstalling(realm, instance);");
        sb.Append("        realm.InstallGlobal(\"").Append(Escape(name)).AppendLine("\", instance);");
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.Append("    static partial void __OnGlobalInstalling(Realm realm, ").Append(model.ClassName).AppendLine(" instance);");
        sb.AppendLine("}");

        ctx.AddSource($"{model.ClassName}.JsObject.Install.g.cs", sb.ToString());
    }

    private static StandaloneGlobalInstallModel? CollectGlobalInstall(GeneratorAttributeSyntaxContext ctx)
    {
        if (ctx.TargetSymbol is not INamedTypeSymbol cls)
            return null;
        ...
        if (ctx.Attributes.Length == 0
            ...
            || ctx.Attributes[0].ConstructorArguments[0].Value is not string globalName)
            return null;

        return new StandaloneGlobalInstallModel(
            cls.ContainingNamespace.IsGlobalNamespace ? null : cls.ContainingNamespace.ToDisplayString(),
            cls.Name,
            globalName);
    }

    private static void EmitStandaloneGlobalInstall(SourceProductionContext ctx, StandaloneGlobalInstallModel model)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("#nullable enable");
        ...
        sb.Append("partial class ").Append(model.ClassName).AppendLine(" : IJsInstallable");
        sb.AppendLine("{");
        sb.AppendLine("    static void IJsInstallable.Install(Realm realm) => __Install(realm);");
        sb.AppendLine("}");
    ...
    private static void EmitConstructor(SourceProductionContext ctx, JsObjectModel model)
    {
        ...
        sb.Append("partial class ").Append(model.ClassName).AppendLine(" : IJsInstallable");
        sb.AppendLine("{");
        sb.AppendLine("    static void IJsInstallable.Install(Realm realm) => __InstallConstructor(realm);");
        sb.AppendLine();
...
#pragma warning restore RS1035
    ...
    private sealed record StandaloneGlobalInstallModel(string? Namespace, string ClassName, string GlobalName);
    ...
    private const string AttributeSource = """
        ...
        [AttributeUsage(AttributeTargets.Class, Inherited = false, AllowMultiple = false)]
        internal sealed class JsGlobalInstallAttribute : Attribute
        {
            public JsGlobalInstallAttribute(string name) { Name = name; }
            public string Name { get; }
        }
        """;
}
