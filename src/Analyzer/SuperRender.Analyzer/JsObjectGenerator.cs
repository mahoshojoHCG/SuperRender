...
public sealed class JsObjectGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(classes, static (ctx, model) => Emit(ctx, model));
    }
    ...
    private static JsObjectModel? Collect(GeneratorAttributeSyntaxContext ctx)
    {
        ...
        if (methods.Count == 0 && properties.Count == 0 && diagnostics.Count == 0)
        {
            return null;
        }
    ...
    private static string WrapReturn(ReturnKind kind, string valueExpr)
    {
        return kind switch
        {
            ...
            ReturnKind.RFloat => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RInt32 => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RInt64 => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RInt16 => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RByte => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RUInt32 => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RUInt64 => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RSByte => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RUInt16 => $"JsNumber.Create((double){valueExpr})",
            ReturnKind.RDecimal => $"JsNumber.Create((double){valueExpr})",
            _ => valueExpr,
        };
    }

    private static void Emit(SourceProductionContext ctx, JsObjectModel model)
    {
    ...
    {
        var target = m.IsStatic ? model.ClassName : "this";

        if (m.IsLegacy)
        {
            sb.Append("                    ").Append(target).Append('.').Append(m.CsharpName).AppendLine(",");
            return;
        }
