...
- `src/Analyzer/SuperRender.Analyzer/` — Roslyn source generator (netstandard2.0). Emits `[JsObject]`/`[JsMethod]`/`[JsProperty]` attrs. For each `[JsObject] partial class : JsObjectBase`, generates switch-based `Get`/`HasProperty` (+optional `Set`) with lazy `JsFunction` caching. Trampolines support **legacy** `(JsValue thisArg, JsValue[] args) => JsValue-derived` or **typed** signatures — params: `JsValue`/`JsObjectBase`/C# primitives/`JsValue[]` rest. `JsObjectBase` params throw `JsTypeError` pre-call; primitives coerce via `ToNumber`/`ToBoolean`/`ToJsString`. Return may be `void`/JsValue-derived/primitive. Diagnostics: JSGEN001 (bad param), JSGEN002 (bad return), JSGEN003 (bad interface member). `function.length` auto-derived — do NOT set `Length =`. Second generator `JsTypeInterfaceProxyGenerator` emits `__<Name>Proxy` (leading `I` stripped) for `IJsType` interfaces, self-registers via `[ModuleInitializer]`. Default JS name = camelCase; override with `[JsName]`. Referenced as `OutputItemType="Analyzer"` by Runtime/Engine/Dom/NodeSimulator/EcmaScript.Tests.
...
**JsValue hierarchy:** `IDynamicMetaObjectProvider` base. `JsObjectBase` (abstract, prototype-chain `Get`/`Set`/`HasProperty`/`Delete`/`ToPrimitive` driven by virtual `GetOwnPropertyDescriptor`/`DefineOwnPropertyCore`; no own storage). `JsDynamicObject` (dictionary + symbols). `JsObject` is a transitional shim `: JsDynamicObject`. Attribute-driven built-ins (`MathObject`, `JsonObject`) inherit `JsObjectBase` directly, property tables from the source generator. `JsCompiler.GetMember`/`SetMember`/`DeleteMember` type-check on `JsObjectBase`.

**Environment** (lexical, TDZ, const), **Realm** (globals + intrinsic prototypes + Eval/FunctionFactory delegates), **Builtins** (32 objects incl. Temporal, Intl, ShadowRealm, TypedArrays, Atomics, SharedArrayBuffer, Proxy, Reflect, WeakRef, FinalizationRegistry). Only `MathObject`/`JsonObject` are generator-driven today; rest use `BuiltinHelper.DefineMethod`/`DefineProperty`.
...
- **Analyzer usage:** `SuperRender.Analyzer` (netstandard2.0) referenced as `OutputItemType="Analyzer" ReferenceOutputAssembly="false"`. Consumers must be `partial` + inherit `JsObjectBase`. `[JsMethod]` supports legacy `(JsValue, JsValue[]) => JsValue-derived` (good for variadics like `Math.max`) or typed signatures. `JsObjectBase` params pre-check; primitives coerce. `function.length` auto-derived — don't pass `Length =`. Symbol-keyed members not yet generated; override `TryGetOwnSymbol` manually.
