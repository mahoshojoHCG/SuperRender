...
- `src/EcmaScript/`
  ...
  - `SuperRender.EcmaScript.Runtime/` — JsValue types, Environment, Realm, Builtins (32 standard library objects), Errors, and `Interop/` (the `IJsType` marker interface, `[JsName]`, `JsValueExtension.AsInterface<T>`, `JsTypeInterfaceProxyRegistry`, and runtime `DispatchProxy` fallback for interface views over arbitrary `JsValue` objects)
...
- `src/Analyzer/`
  - `SuperRender.Analyzer/` — Roslyn source generator project (netstandard2.0). Emits `[JsObject]`/`[JsMethod(name)]`/`[JsProperty(name)]` attributes via `RegisterPostInitializationOutput`, and for every `[JsObject] partial class : JsObjectBase` generates switch-based `Get`/`HasProperty` (+optional `Set`) overrides with lazy `JsFunction` caching. Trampolines support two method shapes: **legacy** `(JsValue thisArg, JsValue[] args) => JsValue-derived` (passed through as a raw method-group reference) and **typed** — per-parameter C# types converted at the JS→C# boundary. Allowed typed parameter types: `JsValue`, `JsObjectBase` (restricted to that level to preserve MonkeyType-style subclass observability), C# primitives (`string`/`bool`/`double`/`float`/integer families/`decimal`), or `JsValue[]` as the final rest-args slot; anything else raises `JSGEN001`. `JsObjectBase` params type-check with a `JsTypeError` thrown **before** the C# method runs; primitive params go through `ToNumber`/`ToBoolean`/`ToJsString` per JS coercion rules. Return types may be `void` (→ `undefined`), any JsValue-derived type, or any listed primitive; unsupported returns raise `JSGEN002`. `function.length` is auto-derived from the signature (typed: count of non-rest params; legacy: 0) — `JsMethodAttribute` no longer carries `Length`. A second generator `JsTypeInterfaceProxyGenerator` discovers user interfaces inheriting `IJsType` (see Runtime's `Interop/` folder) and emits `__<Name>Proxy` classes (leading `I` stripped from the interface name) that forward member access to a backing `JsObjectBase` — primary path for `value.AsInterface<T>()`. Proxies self-register via `[ModuleInitializer]` into `JsTypeInterfaceProxyRegistry`. Unsupported interface members (events, indexers, generic methods, ref/out params, non-convertible types) raise `JSGEN003`. Default JS member name = `JsonNamingPolicy.CamelCase`-equivalent (manual impl inside the generator); override with `[JsName("custom")]`. Referenced by Runtime/Engine/Dom/NodeSimulator/EcmaScript.Tests with `OutputItemType="Analyzer"` so consumers can define `IJsType` interfaces anywhere in the EcmaScript stack.
- `src/Browser/`
...
- `JsValue` hierarchy — `IDynamicMetaObjectProvider` base. Object layer split into `JsObjectBase` (abstract; prototype-chain-aware `Get`/`Set`/`HasProperty`/`Delete`/`ToPrimitive` driven by the virtual hooks `GetOwnPropertyDescriptor`/`DefineOwnPropertyCore`; no own storage) and `JsDynamicObject` (dictionary-backed `DefineOwnProperty`/`GetOwnProperty`/`OwnPropertyKeys` + symbol props). `JsObject` is a transitional shim `: JsDynamicObject` so `JsFunction`/`JsArray`/`JsRegExp`/`JsGeneratorObject`/wrappers/module objects keep inheriting through it unchanged. Attribute-driven built-ins (`MathObject`, `JsonObject`) inherit `JsObjectBase` directly and get their property table from the `SuperRender.Analyzer` source generator via `[JsObject]`/`[JsMethod]`/`[JsProperty]`. `JsCompiler.GetMember`/`SetMember`/`DeleteMember` type-check on `JsObjectBase` so attribute-driven classes participate in member access.
...
- `Builtins` — 32 standard library objects (Object, Array, String, Number, Math, JSON, Date, RegExp, Map, Set, Promise, Proxy, Reflect, BigInt, WeakRef, FinalizationRegistry, Iterator, ArrayBuffer, SharedArrayBuffer, TypedArrays, Atomics, Intl, Temporal, ShadowRealm, structuredClone, etc.). `MathObject` and `JsonObject` are generator-driven partials (`[JsObject] : JsObjectBase`) and serve as the reference for migrating more built-ins; the rest still inherit `JsObject` and register members imperatively via `BuiltinHelper.DefineMethod`/`DefineProperty`.
...
**JS → C# structural binding (`IJsType`):** user-defined interfaces inheriting `SuperRender.EcmaScript.Runtime.Interop.IJsType` can be used as a typed view over any `JsValue` via `value.AsInterface<T>()`. Flow: (1) non-`JsObjectBase` throws `JsTypeError`; (2) if the backing object already implements `T` (e.g., `[JsObject] partial class Foo : JsObjectBase, IFoo`), it is returned as-is (fast path); (3) otherwise the `SuperRender.Analyzer` source generator's per-interface `__<Name>Proxy` class — leading `I` stripped (`IFoo` → `__FooProxy`) — registered via `[ModuleInitializer]` into `JsTypeInterfaceProxyRegistry`, is instantiated; (4) falls back to `System.Reflection.DispatchProxy` when no generated proxy is registered (not AOT-safe). Proxies are cached 1:1 per `(JsObjectBase, T)` via a `ConditionalWeakTable`. Allowed member types on the interface: `JsValue` and `JsValue`-derived pass-through, C# primitives (coerced via `ToNumber`/`ToBoolean`/`ToJsString`), other `IJsType` interfaces (recursively wrapped / unwrapped via `IJsTypeProxy.Target`), `JsValue[]` as a final rest parameter. Unsupported shapes (events, indexers, generic methods, `ref`/`out`, non-convertible types) raise `JSGEN003`. Default JS name = `JsonNamingPolicy.CamelCase`-equivalent; override with `[JsName("custom")]`.
...
- `SuperRender.Analyzer` (source generator) targets `netstandard2.0`, is referenced by Runtime as `OutputItemType="Analyzer" ReferenceOutputAssembly="false"`. Consuming classes must be `partial` and inherit `JsObjectBase`. `[JsMethod]` methods can use either the legacy raw signature `(JsValue thisArg, JsValue[] args) => JsValue-derived` (still useful for variadic built-ins like `Math.max`/`min`/`hypot`) or a typed signature — each parameter is `JsValue` / `JsObjectBase` / a C# primitive / `JsValue[]` (rest). `JsObjectBase` params enforce a pre-call `JsTypeError`; primitive params follow JS coercion. `function.length` is auto-derived; do not add `Length =` to the attribute. Symbol-keyed members (`Symbol.toStringTag`, etc.) are not yet handled by the generator — override `TryGetOwnSymbol` manually
